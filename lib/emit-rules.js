 ...
 * @verified-by test/emit-rules.test.js
...
export const RUNTIMES_VALIDES = ['claude-code', 'cursor', 'codex', 'copilot', 'gemini', 'kiro', 'all'];
...
/**
 * Gouvernance advisory en pull : chaque règle `.claude/rules/*.md` n'est chargée
 * que sur les fichiers matchant son `paths:` (primitive native Claude Code).
 *
 * @intent INTENT-005
 * @spec SPEC-005-1-context-pull
 * @verified-by test/emit-rules-pull.test.js
 * @governance AIAD-RGESN
 */
 ...
 * @governance AIAD-RGESN
 */
function genererKiroSteering({ identite, regles, intent, version, sourceHash }) {
  const fm = [
    '---',
    'inclusion: always',
    intent ? `# intent_id: ${intent.id}` : null,
...
function genererKiroTier1(agentId, contenuSource, { intent, version, sourceHash }) {
  const globs = GLOBS_TIER1[agentId] || ['**/*'];
  const pattern = Array.isArray(globs) && globs.length === 1 ? globs[0] : JSON.stringify(globs);
  const fm = [
    '---',
    'inclusion: fileMatch',
...
# ${agentId} — Kiro steering (Tier 1, droit de veto)
...
`);
  ...
  // 7. Kiro (AWS Bedrock AI IDE) — steering principal + 4 fichiers Tier 1
  if (wants('kiro')) {
    console.log(`\n${C.gras}  Kiro (.kiro/steering/)${C.reset}\n`);
    ecrireOuVerifier(
      join(projetDir, '.kiro', 'steering', 'aiad.md'),
      genererKiroSteering(ctx),
      { check, projetDir, stats, dryRun }
    );
    for (const agentId of ['AIAD-RGPD', 'AIAD-RGAA', 'AIAD-AI-ACT', 'AIAD-RGESN']) {
      if (!governance[agentId]) {
        logSkip(`.kiro/steering/aiad-${nomRule(agentId)}.md`, 'gouvernance absente');
        continue;
      }
      ecrireOuVerifier(
        join(projetDir, '.kiro', 'steering', `aiad-${nomRule(agentId)}.md`),
        genererKiroTier1(agentId, governance[agentId], ctx),
        { check, projetDir, stats, dryRun }
      );
    }
  }
