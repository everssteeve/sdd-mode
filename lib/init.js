...
export async function init(projetDir, options = {}) {
  ...
  if (minimal) {
    return initMinimal(projetDir, { force, version });
  }
...
`);
  ...
  // 5. GitHub Actions (CI traceability — Évolution #6 v1.10)
  ...
  // 6. Hooks Git (optionnel)
  if (withGitHooks) {
    await installerHooks(projetDir, { force });
  }

  // 6.5. Multi-runtime — Évolution #8 (v1.12)
  // Par défaut, on émet AGENTS.md même en mode claude-code seul (Copilot le lit
  // nativement). Pour une cible explicite cursor/codex/copilot/gemini ou all,
  // on génère aussi les fichiers dérivés.
  ...
  if (aBesoinEmit) {
    ...
    try {
      await emitRules(projetDir, { runtimes });
    } catch (err) {
...
async function initMinimal(projetDir, { force, version }) {
  console.log(`
...
`);
  ...
  // Copier AGENT-GUIDE + index
  copierDossierRecursif(join(TEMPLATES_MIN_DIR, '.aiad'), aiadDir, force);

  // 2. Commandes Claude Code — 4 fichiers seulement
  ...
  if (existsSync(claudeMdDest) && !force) {
    ...
    if (!contenuExistant.includes('# SDD Mode')) {
      const sddSection = readFileSync(claudeMdSource, 'utf-8');
      writeFileSync(claudeMdDest, contenuExistant + '\n\n' + sddSection, 'utf-8');
      log(`${COULEURS.vert}+${COULEURS.reset}`, 'CLAUDE.md — section SDD Mode minimal ajoutée');
    } else {
      logExiste('CLAUDE.md (section SDD Mode déjà présente)');
    }
  } else {
