...
const SOUS_DOSSIERS = ['intents', 'specs', 'facts'];
...
export const TYPES_ARTEFACTS = [
  ...
  { kind: 'facts', prefixes: ['FACT-'], format: 'FACT-NNN-slug' },
];
...
export function detecterSousDossier(id) {
  ...
  throw new Error(`ID inconnu : "${id}". Attendu : INT-NNN, INTENT-NNN, SPEC-NNN-N-slug ou FACT-NNN-slug.`);
}
...
const STATUTS_LIVRES = new Set(['done', 'delivered', 'livre', 'livré', 'closed', 'clos', 'résolu']);
