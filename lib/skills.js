...
// Plafond aligné sur le réglage `skillListingMaxDescChars` (settings.json,
// défaut 1536, §3.7). Au-delà, Claude Code tronque la description dans le
// listing à froid → déclenchement dégradé + budget de contexte gaspillé.
/**
 * Garde-fou de budget : toute description de skill au-delà du plafond est
 * signalée par `validerSkill` (tronquée à froid).
 *
 * @intent INTENT-005
 * @spec SPEC-005-1-context-pull
 * @verified-by test/skills.test.js
 */
...
export function validerSkill(cheminSkillMd) {
  ...
  if (desc === '') {
    ...
    raisons.push(`Description trop longue (${desc.length} caractères, plafond ${MAX_DESCRIPTION} = skillListingMaxDescChars) — tronquée à froid (§3.7).`);
  }
