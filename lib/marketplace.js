...
export function afficherListe(filters = {}) {
  ...
  if (filters.json) {
    process.stdout.write(JSON.stringify({
      ...
      filters: { secteur: filters.secteur || null, juridiction: filters.juridiction || null },
    }, null, 2) + '\n');
    return packs;
  }
  logHeader(
 ...
 */
export function afficherInfo(id, options = {}) {
  const pack = trouverPack(id);
  if (!pack) {
    const ids = listerPacksPremium().map((p) => p.id).join(', ');
    if (options.json) {
      process.stdout.write(JSON.stringify({ id, found: false, available: ids.split(', ') }, null, 2) + '\n');
      return null;
    }
  ...
  if (options.json) {
    process.stdout.write(JSON.stringify({ id, found: true, pack }, null, 2) + '\n');
    return pack;
  }
