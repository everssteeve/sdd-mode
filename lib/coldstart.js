...
export function bench(projetDir = process.cwd(), options = {}) {
  const { json = false } = options;
  const commandsDir = join(projetDir, '.claude', 'commands');
  if (!existsSync(commandsDir)) {
    if (json) {
      process.stdout.write(JSON.stringify({ error: '.claude/commands/ absent', projetDir }) + '\n');
      process.exit(1);
    }
  ...
  const result = {
    ...
    apresTokens,
    reductionFinalePct,
    reductionTransitionPct,
    routers: { count: routers.length, frontmatterBytes: routers.reduce((s, c) => s + c.frontmatterBytes, 0) },
    alias: { count: alias.length, frontmatterBytes: alias.reduce((s, c) => s + c.frontmatterBytes, 0) },
    subSdd: { count: subSdd.length, totalBytes: subSdd.reduce((s, c) => s + c.totalBytes, 0) },
    subAiad: { count: subAiad.length, totalBytes: subAiad.reduce((s, c) => s + c.totalBytes, 0) },
