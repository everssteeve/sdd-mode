...
`);
  ...
  if (existsSync(hookSource)) {
    const contenu = readFileSync(hookSource, 'utf-8');
    const result = mettreAJour(hookDest, contenu);
    const chemin = rel(hookDest);
    if (result === 'created') { logCreation(chemin); stats.created++; }
    else if (result === 'updated') { logMaj(chemin); stats.updated++; }
    else { logOk(chemin); stats.unchanged++; }
  }
  ...
  // (l'install initiale se fait via `aiad-sdd init --with-git-hooks` ou `aiad-sdd hooks`.)
  ...
  if (existsSync(preCommitSource) && existsSync(preCommitDest)) {
    const contenu = readFileSync(preCommitSource, 'utf-8');
    const result = mettreAJour(preCommitDest, contenu);
    if (result === 'updated') {
      try { chmodSync(preCommitDest, 0o755); } catch {}
      logMaj(rel(preCommitDest)); stats.updated++;
    } else if (result === 'unchanged') {
      logOk(rel(preCommitDest)); stats.unchanged++;
    }
  }
  ...
  if (existsSync(settingsSource)) {
    ...
    const result = mettreAJour(settingsDest, nouveauContenu);
    const chemin = rel(settingsDest);
    if (result === 'created') { logCreation(chemin); stats.created++; }
    else if (result === 'updated') { logMaj(chemin); stats.updated++; }
    else { logOk(chemin); stats.unchanged++; }
  }
  ...
  const nouveauxDossiers = ['facts', 'metrics', 'metrics/security', 'metrics/audit'];
