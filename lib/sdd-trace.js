...
//
// @intent INTENT-024
// @spec SPEC-024-1-trace-exemption
// @intent INTENT-022
...
// Détecte les fichiers lib/*.js ajoutés dans le diff courant (--diff-filter=A).
// Utilisé pour qualifier les gaps codeSansSpec en bloquant vs non-bloquant.
// En l'absence de git ou en cas de timeout (2 s), retourne un Set vide — les
// gaps sont alors tous non-bloquants (fail-open sur cette vérification seule).
// cwd : répertoire de travail git (défaut = process.cwd()). Injectez racineProjet
// pour que les chemins retournés soient relatifs à la même racine que scanCode.
export function detecterNouveauxFichiers(libDir, cwd) {
  try {
    const opts = { encoding: 'utf-8', timeout: 2000, stdio: ['ignore', 'pipe', 'ignore'] };
    if (cwd) opts.cwd = cwd;
    const r = spawnSync('git', ['diff', '--name-only', '--diff-filter=A', 'HEAD', '--', libDir], opts);
    if (r.status !== 0 || r.error) return new Set();
    return new Set((r.stdout || '').trim().split('\n').filter(Boolean).filter((p) => p.endsWith('.js') && !p.endsWith('.test.js')));
  } catch { return new Set(); }
}

// (#261) Enveloppe avec _meta pour exports JSON (stdout/file/sarif).
// schema sous-namespace `aiad-sdd-trace` ; les SARIF restent inchangés
// (spec OASIS stricte, pas d'enrichissement custom).
function enveloperAvecMeta(matrice) {
  return { _meta: buildMeta({ schema: 'aiad-sdd-trace' }), ...matrice };
}
...
//
// Acceptés dans (exemples ci-dessous neutralisés via `aiad-trace-ignore`) :
//   - JSDoc           /** @spec SPEC-042-1-flow-auth */   aiad-trace-ignore
//   - Commentaires    // @spec SPEC-042-1-flow-auth       aiad-trace-ignore
//   - Docstrings Py   """ @spec SPEC-042-1-flow-auth """  aiad-trace-ignore
//   - Hash Python     # @spec SPEC-042-1-flow-auth        aiad-trace-ignore
//
// Format ID :
//   INTENT-\d{3,}(-[a-z0-9-]+)?
//   SPEC-\d{3,}(-\d+)?(-[a-z0-9-]+)?

// Les IDs réels sont strictement numériques (INTENT-\d{3,}, SPEC-\d{3,}-\d+).
// Resserrer le format évite que la doc/les templates de l'outil — qui contiennent
// des *placeholders* (`SPEC-NNN-N-slug`, `SPEC-XXX-N-slug`, `SPEC-NNN-x`…) — soient
// captés comme de vraies annotations lors d'un self-scan. Pour les exemples au
// format valide (ex. `SPEC-042-1-flow-auth` dans un commentaire de doc), poser le
// marqueur `aiad-trace-ignore` sur la ligne (cf. parserAnnotations).
export const ANNOTATION_REGEX = {
  intent: /@intent\s+(INTENT-\d{3,}(?:-[a-z0-9-]+)?)/g,
  spec: /@spec\s+(SPEC-\d{3,}-\d+(?:-[a-z0-9-]+)?)/g,
...
const EXTENSIONS_CODE = new Set([
  ...
  '.ex', '.exs',         // Elixir
  // CI/config — annotations @spec dans les commentaires YAML/TOML.
  '.yml', '.yaml',
]);
...
export function parserAnnotations(contenu, cheminRelatif) {
  ...
  for (let i = 0; i < lignes.length; i++) {
    ...
    // Échappatoire pour la doc/les exemples au format d'ID valide : une ligne
    // marquée `aiad-trace-ignore` n'est jamais scannée pour des annotations.
    if (ligne.includes('aiad-trace-ignore')) continue;
...
function lireSpecs(racineProjet) {
  ...
  for (const nom of readdirSync(dir)) {
    ...
    const formatFromFm = data.format || data.Format;
    // nettoyerMd strips bold markers, so "**Format** : EARS" → "Format : EARS" in contenu
    const formatM = !formatFromFm && contenu.match(/^Format\s*:\s*([A-Z]+)/im);
    // Exemption de traçabilité (SPEC-024-1) : une SPEC sans code applicatif
    // annotable (livrable doc/site/CI) peut être marquée `done` sans déclencher
    // le faux gap `specsValideesNonImplementees`, à condition de déclarer
    // explicitement `traceability: exempt` + une raison non vide. Clés brutes
    // (snake_case) — parseFrontmatter ne camélise pas (cf. data.parent_intent).
    out.push({
      id: m[1],
      file: relative(racineProjet, chemin),
      title: data.title || data.titre || (titreM ? titreM[1].trim() : m[1]),
      ...
      format: String(formatFromFm || (formatM ? formatM[1] : 'prose')).toUpperCase(),
      traceability: String(data.traceability || '').toLowerCase(),
      traceabilityReason:
        typeof data.traceability_reason === 'string' ? data.traceability_reason : '',
    });
  }
  return out;
}
...
export function construireMatrice(racineProjet) {
  ...
  // Exemption de traçabilité (SPEC-024-1) : SPEC sans code applicatif annotable
  // mais réellement livrée, déclarée `traceability: exempt` + raison non vide.
  // Fail-honest : l'exemption sans raison est inerte (le gap est maintenu).
  const estExempte = (s) => s.traceability === 'exempt' && s.traceabilityReason.trim().length > 0;
  ...
  const gaps = {
    ...
    // `in-progress` est exclu : une SPEC en cours d'implémentation est un WIP
    // assumé, pas un drift bloquant. Elle reste visible (non bloquante) via
    // `specsSansCode`. Seuls les statuts « figés » (prêt / en validation / fait)
    // sans code annoté constituent un gap bloquant — sauf exemption explicite.
    // @spec SPEC-031-1-hook-stop-ready-fix
    // `ready` exclu : gate passée mais exec non démarré = état normal, pas un drift.
    specsValideesNonImplementees: specs
      .filter((s) => ['validation', 'done'].includes(s.status))
      .filter((s) => !codeParSpec.has(shortSpecId(s.id)))
      .filter((s) => !estExempte(s)),
    codeSansSpec: (() => {
      const nouveauxFichiers = detecterNouveauxFichiers('lib/', racineProjet);
      const items = codeFiles
        .filter((f) => f.annotations.specs.length === 0)
        .map((f) => ({
          ...f,
          isNewFile: nouveauxFichiers.has(f.path),
          severity: nouveauxFichiers.has(f.path) ? 'bloquant' : 'non-bloquant',
        }));
      const bloquant = items.filter((f) => f.severity === 'bloquant').length;
      return { bloquant, non_bloquant: items.length - bloquant, total: items.length, items };
    })(),
    ...
    // @spec SPEC-019-2-trace-ears-gap — SPECs EARS actives sans tests liés
    earsSpecsSansTests: specs
      .filter((s) => s.format === 'EARS')
      .filter((s) => !['draft', 'archived'].includes(s.status))
      .filter((s) => !estExempte(s))
      .filter((s) => {
        const key = shortSpecId(s.id);
        const testsViaSpec = testsParSpec.get(key) || [];
        const code = codeParSpec.get(key) || [];
        const testsViaCode = code.flatMap((c) => testsParPathCode.get(c.path) || []);
        return testsViaSpec.length === 0 && testsViaCode.length === 0;
      }),
  };
  ...
  // Les artefacts archivés sont des références valides : une annotation @intent/@spec
  // pointant vers un artefact archivé n'est pas un orphelin — l'intent existe,
  // il est simplement clôturé. On enrichit les deux sets avec les IDs de archive/.
  // @spec SPEC-026-1-archive-done
  for (const kind of ['intents', 'specs']) {
    const archiveDir = join(racineProjet, '.aiad', kind, 'archive');
    if (!existsSync(archiveDir)) continue;
    for (const nom of readdirSync(archiveDir)) {
      if (!nom.endsWith('.md') || nom.startsWith('_')) continue;
      const mIntent = nom.match(/^(INTENT-\d+)/i);
      if (mIntent) intentsConnus.add(mIntent[1].toUpperCase());
      const mSpec = nom.match(/^(SPEC-\d+-\d+)/i);
      if (mSpec) specsConnus.add(mSpec[1].toUpperCase());
    }
  }
  for (const f of codeFiles) {
    for (const s of f.annotations.specs) {
  ...
  return {
    ...
    backward,
    gaps,
    // SPECs exemptées de traçabilité (SPEC-024-1) — exposées pour visibilité,
    // hors du décompte de gaps (ce ne sont pas des drifts).
    specsExemptees: specs.filter(estExempte),
  };
}
...
function rendreMarkdown(modele) {
  ...
  L.push(`- SPECs validées sans code (statut ready/validation/done) : **${modele.gaps.specsValideesNonImplementees.length}**`);
  for (const x of modele.gaps.specsValideesNonImplementees) L.push(`  - ${x.id} (statut : ${x.status})`);
  L.push('');
  L.push(`### EARS sans tests`);
  L.push(`- SPECs EARS sans tests liés (statut ≠ draft/archived) : **${modele.gaps.earsSpecsSansTests.length}**`);
  for (const x of modele.gaps.earsSpecsSansTests) L.push(`  - ${x.id} (EARS, statut : ${x.status})`);
  L.push('');
  L.push(`### Non-tracés`);
  L.push(`- Code sans \`@spec\` : **${modele.gaps.codeSansSpec.total}** (bloquant : ${modele.gaps.codeSansSpec.bloquant}, non-bloquant : ${modele.gaps.codeSansSpec.non_bloquant})`);
  for (const f of modele.gaps.codeSansSpec.items.slice(0, 50)) L.push(`  - ${f.path}${f.severity === 'bloquant' ? ' ⚠ nouveau fichier' : ''}`);
  if (modele.gaps.codeSansSpec.total > 50) L.push(`  - … (+${modele.gaps.codeSansSpec.total - 50} autres)`);
...
<div class="stats">
  ...
  <div class="stat"><div class="v">${modele.gaps.codeSansSpec.total}</div><div class="k">Code sans @spec</div></div>
...
export async function trace(projetDir, options = {}) {
  ...
  if (jsonStdout) {
    process.stdout.write(JSON.stringify(enveloperAvecMeta(modele), null, 2) + '\n');
    return modele;
  }
  ...
  if (formats.includes('json')) {
    ...
    // (#305) Trailing newline POSIX
    writeFileSync(p, JSON.stringify(enveloperAvecMeta(modele), null, 2) + '\n', 'utf-8');
    ecrits.push(p);
  }
  ...
  if (verbose) {
    ...
    console.log(`    Code sans @spec                        : ${couleurGap}${modele.gaps.codeSansSpec.total}${C.reset} (bloquant : ${modele.gaps.codeSansSpec.bloquant})`);
    console.log(`    Code annoté sans tests                 : ${couleurGap}${modele.gaps.codeSansTests.length}${C.reset}`);
    console.log(`    SPECs EARS sans tests liés             : ${couleurGap}${modele.gaps.earsSpecsSansTests.length}${C.reset}`);
  ...
  if (suggest) {
    ...
    if (verbose) {
      ...
      if (modele.gaps.earsSpecsSansTests.length > 0) {
        console.log(`  ${C.gras}Tests manquants — squelettes à générer${suffixe}${C.reset}`);
        for (const spec of modele.gaps.earsSpecsSansTests) {
          console.log(`    ${C.cyan}→${C.reset} npx aiad-sdd suggest-tests ${spec.id}`);
        }
        console.log('');
      }
    }
    ...
    const stop = demarrerWatch(projetDir, async () => {
      try {
        ...
        for (const f of formats) {
          ...
          else if (f === 'json') writeFileSync(join(outDir, 'trace.json'), JSON.stringify(enveloperAvecMeta(m), null, 2) + '\n', 'utf-8');
...
// une fonction `stop()`.
export function demarrerWatch(projetDir, onChange, { debounceMs = 200, pollMs = 60 } = {}) {
  const aiadDir = join(projetDir, '.aiad');
  let timer = null;
  const trigger = () => {
  ...
  // `fs.watch({ recursive: true })` n'est pas supporté sur Linux avant Node 20
  // (il lève `ERR_FEATURE_UNAVAILABLE_ON_PLATFORM`). On tente d'abord le watch
  // natif récursif (performant) ; si aucun watcher natif n'a pu être créé, on
  // bascule sur un polling de signature d'arborescence (fallback portable).
  let watcherAiad, watcherRoot, pollTimer;
  let nativeOk = false;

  // `fs.watch({ recursive: true })` n'est fiable que sur macOS/Windows, et sur
  // Linux à partir de Node 20. Sur Linux + Node < 20, l'option est ignorée (ou
  // l'erreur est émise de façon asynchrone) : aucune exception synchrone à
  // catcher → on détecte explicitement la config non supportée pour basculer
  // d'emblée sur le polling. `AIAD_WATCH_POLL=1` force aussi ce mode.
  const majeurNode = Number(process.versions.node.split('.')[0]);
  const recursifSupporte = process.platform !== 'linux' || majeurNode >= 20;
  const forcerPolling = process.env.AIAD_WATCH_POLL === '1' || !recursifSupporte;
  ...
  if (!forcerPolling) {
    ...
    try {
      ...
      nativeOk = true;
    } catch { /* ENOENT possible si init en cours, ou récursif non supporté */ }
    ...
    try {
      ...
      nativeOk = true;
    } catch { /* récursif non supporté → fallback polling ci-dessous */ }
  }

  // Fallback polling (Linux + Node < 20, ou plateformes sans watch récursif).
  if (!nativeOk) {
    let signaturePrec = signatureArbreWatch(projetDir, aiadDir, ignorer);
    pollTimer = setInterval(() => {
      const sig = signatureArbreWatch(projetDir, aiadDir, ignorer);
      if (sig !== signaturePrec) {
        signaturePrec = sig;
        trigger();
      }
    }, pollMs);
...
/**
 * Signature d'arborescence pour le fallback polling de `demarrerWatch` :
 * concatène `chemin:mtimeMs:taille` pour tout fichier non ignoré de `.aiad/`
 * et tout fichier code reconnu sous la racine projet. Un changement de
 * contenu/ajout/suppression modifie la signature → déclenche `onChange`.
 *
 * @param {string} projetDir
 * @param {string} aiadDir
 * @param {(filename: string) => boolean} ignorer prédicat sur chemin relatif
 * @returns {string}
 */
function signatureArbreWatch(projetDir, aiadDir, ignorer) {
  const parts = [];
  const visiter = (dir, base, codeUniquement) => {
    let entries;
    try { entries = readdirSync(dir, { withFileTypes: true }); } catch { return; }
    for (const ent of entries) {
      const rel = base ? `${base}/${ent.name}` : ent.name;
      if (ignorer(rel) || ignorer(`${ent.name}`)) continue;
      const chemin = join(dir, ent.name);
      if (ent.isDirectory()) {
        if (ent.name === 'node_modules' || ent.name === '.git') continue;
        visiter(chemin, rel, codeUniquement);
      } else if (ent.isFile()) {
        if (codeUniquement && !EXTENSIONS_CODE.has(extname(ent.name))) continue;
        let st;
        try { st = statSync(chemin); } catch { continue; }
  ...
  if (existsSync(aiadDir)) visiter(aiadDir, '', false);
  visiter(projetDir, '', true);
...
function compterGapsBloquants(m) {
  ...
  // SPEC-022-2 : nouveaux fichiers lib/ sans @spec sont bloquants.
  return (
    m.gaps.specsValideesNonImplementees.length +
    m.gaps.specsOrphelinsSurCode.length +
    m.gaps.intentsOrphelinsSurCode.length +
    m.gaps.earsSpecsSansTests.length +
    m.gaps.codeSansSpec.bloquant
  );
}
