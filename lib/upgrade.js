...
import { C, log, logCreation, logMaj, logPreserve } from './term.js';
...
// Wrapper local — libellé "(déjà à jour)" différent du term.js standard
// "(à jour)". À unifier en #4 sans casser l'UX existante.
...
// GitHub Action — déployée avec le module 'all' (CI optionnelle pour les autres profils)
const CI_FILES = [
  '.github/workflows/sdd-trace.yml',
  '.github/workflows/aiad-emit-rules-check.yml',
];

// Toutes les commandes restantes (init/exec/validate/resume/split SDD + intent/spec/gate/drift-check/trace version router)
...
// Skills réutilisables (v1.9+) — auto-déclenchées par Claude Code, invoquées par les commandes SDD
const SKILLS_FILES = [
  ...
  '.claude/skills/reasons-canvas/SKILL.md',
...
async function upgradeRituals(projetDir, stats, { force }) {
  ...
  // Skills réutilisables (déployées dès qu'on quitte le minimal)
  copierListe(projetDir, SKILLS_FILES, stats, { force });
  // Index des intents/specs (versions full = ajoutent statuts détaillés)
  // + Template EARS optionnel (v1.11) — installé dès qu'on quitte le minimal
  copierListe(projetDir, [
    '.aiad/intents/_index.md',
    '.aiad/specs/_index.md',
    '.aiad/specs/spec-ears-template.md',
...
async function upgradeAll(projetDir, stats, { force }) {
  ...
  console.log(`\n${C.gras}  Module : GitHub Action (CI traceability)${C.reset}\n`);
  copierListe(projetDir, CI_FILES, stats, { force });
