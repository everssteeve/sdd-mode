...
export function pageTraceability(donnees) {
  ...
  const gapsTotal = m.gaps.intentsSansSpec.length + m.gaps.specsSansCode.length
    ...
    + m.gaps.intentsOrphelinsSurCode.length + m.gaps.codeSansSpec.total
...
<section class="matrix-tab">
  ...
  ${m.forward.length === 0
    ...
    : `<div class="filter"><input type="search" id="qFwd" data-filter-target="tFwd" placeholder="Filtrer la matrice forward…"/></div>
       <table id="tFwd" data-a11y-caption="Matrice de traçabilité — Forward (Intent → SPEC → Code → Tests)">
...
<section class="matrix-tab">
  ...
  ${m.backward.length === 0
    ...
    : `<div class="filter"><input type="search" id="qBwd" data-filter-target="tBwd" placeholder="Filtrer la matrice backward…"/></div>
       <table id="tBwd" data-a11y-caption="Matrice de traçabilité — Backward (Tests → Code → SPEC → Intent)">
  ...
  <div class="split">
    ...
    <div class="card">
      ...
      <ul class="compact">
        ...
        <li><strong>${m.gaps.codeSansSpec.total}</strong> fichier(s) code sans <code>@spec</code> (bloquant : ${m.gaps.codeSansSpec.bloquant})</li>
