...
const BUCKETS = [
  { cle: 'neuf', label: '0-7j', max: 7, couleur: '#2b8a3e' },
  { cle: 'recent', label: '8-30j', max: 30, couleur: '#4c6ef5' },
  { cle: 'mature', label: '31-90j', max: 90, couleur: '#f5a623' },
  { cle: 'ancien', label: '91-180j', max: 180, couleur: '#e8590c' },
...
export function blocBacklogPyramid(donnees) {
  ...
  const details = p.buckets.filter((b) => b.count > 0).map((b) => {
    ...
    return `<div class="bp-bucket">
      <div class="bp-bucket-h" style="color:${b.couleur}">${escape(b.label)} — ${b.count} Intent(s)</div>
      ${rows}
    </div>`;
  }).join('');
  return `${BP_CSS}<section>
