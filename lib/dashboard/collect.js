...
// (#017-4) Parse les critères d'acceptation EARS d'une SPEC (sections ### CA-NNN).
// Retourne un tableau [{id, titre, pattern, texte}] — vide si §3 absente.
function extraireCriteresAcceptation(body) {
  const section3 = extraireSection(body, '3.');
  if (!section3) return [];
  const lignes = section3.split(/\r?\n/);
  const cas = [];
  let current = null;
  for (const l of lignes) {
    const mh = l.match(/^###\s+(CA-\d+[a-z]?)\s*(?:[—-]\s*(.+))?$/);
    if (mh) {
      if (current) cas.push(current);
      current = { id: mh[1], titre: (mh[2] || '').trim(), pattern: null, texte: null };
    } else if (current) {
      const mp = l.match(/^>\s*Pattern\s*:\s*(.+)$/);
      if (mp) { current.pattern = mp[1].trim(); continue; }
      // Texte EARS dans un code-span de toute la ligne
      const mt = l.match(/^`(.+)`\s*$/);
      if (mt && current.texte === null) {
        current.texte = mt[1].replace(/\\`/g, '`');
      }
    }
  }
  if (current) cas.push(current);
...
// (#017-4) Parse les items de liste de la section §5 Dépendances.
function extraireDependances(body) {
  const sec = extraireSection(body, '5.');
  if (!sec) return [];
  return sec.split(/\r?\n/)
    .filter((l) => /^\s*-\s+/.test(l))
...
// (#017-4) Parse les items de checklist de la section §7 DoOD.
function extraireDood(body) {
  const sec = extraireSection(body, '7.');
  if (!sec) return [];
  return sec.split(/\r?\n/)
    .filter((l) => /^\s*-\s+\[[ xX]\]/.test(l))
    .map((l) => {
      const checked = /\[[xX]\]/.test(l);
      const texte = l.replace(/^\s*-\s+\[[ xX]\]\s*/, '').replace(/`/g, '').trim();
      return { checked, texte };
    });
}
...
export function lireIntents(racineProjet) {
  ...
  return listerFichiersMd(dir).map((chemin) => {
    ...
    // (#426/#427/#428) Champs PM directement lisibles depuis le frontmatter.
    // Exposés sur l'objet Intent pour que les modules priority/roadmap/
    // outcome-mapping puissent les consommer sans re-parser le fichier.
    const pmFields = {};
    for (const k of ['priority', 'Priority', 'rice', 'RICE', 'wsjf', 'WSJF',
      'wave', 'Wave', 'target', 'target_date', 'targetDate', 'quarter', 'Quarter',
      'personas', 'persona', 'user_stories', 'userStories', 'us',
      'outcomes', 'outcome',
      'depends_on', 'dependsOn', 'depends', 'blocked_by', 'blockedBy',
      'owner', 'Owner', 'owners', 'Owners', 'pm', 'PM',
      'assignee', 'assignees', 'responsable',
      'sponsor', 'Sponsor', 'sponsors', 'Sponsors',
      'stakeholder', 'stakeholders', 'Stakeholders',
      'business_owner', 'businessOwner',
      'risks', 'Risks', 'risque', 'risques', 'Risques', 'concerns',
      'risk_level', 'riskLevel', 'niveau_risque',
      'hypothesis', 'hypothese', 'hypothèse', 'Hypothesis',
      'hypothesis_status', 'hypothesisStatus', 'hypothese_status',
      'okr', 'OKR', 'okrs', 'objective', 'Objective', 'objectives',
      'key_results', 'keyResults',
      'kind', 'Kind', 'track', 'Track',
      'tags', 'Tags', 'labels', 'Labels', 'etiquettes',
      'confidence', 'Confidence', 'confidence_level', 'confidenceLevel',
      'ai_risk', 'aiRisk', 'ai_act_category', 'aiActCategory',
      'experiment', 'Experiment', 'ab_test', 'abTest',
      'risks_accepted', 'risksAccepted', 'accepted_risks', 'acceptedRisks', 'risk_status', 'riskStatus']) {
      if (data && data[k] !== undefined) pmFields[k] = data[k];
    }
    return {
      ...
      executor: lireChampPriorite(contenu, 'executor') ?? null,
      validator: lireChampPriorite(contenu, 'validator') ?? null,
      ...pmFields,
...
export function lireSpecs(racineProjet) {
  ...
  return fichiers.map((chemin) => {
    ...
    const rawId = m ? m[1] : basename(chemin, '.md');
    return {
      id: rawId,
      slug: rawId.replace(/^SPEC-/i, ''),
      file: relative(racineProjet, chemin),
      titre: lireChampPriorite(contenu, 'title', 'titre') || extraireTitre(body) || rawId,
      ...
      governanceTags: (() => {
        const gm = body.match(/@governance\s+([\w,\s-]+)/);
        return gm ? gm[1].split(',').map((g) => g.trim()).filter(Boolean) : [];
      })(),
      parallelWith,
      auteur: lireChampPriorite(contenu, 'auteur', 'Auteur', 'author'),
      date: lireChampPriorite(contenu, 'date', 'Date'),
      mtime,
      contexte: extraireSection(body, '1.') || null,
      criteresAcceptation: extraireCriteresAcceptation(body),
      interface_: extraireSection(body, '4.') || null,
      dependances: extraireDependances(body),
      dood: extraireDood(body),
    };
  });
}
