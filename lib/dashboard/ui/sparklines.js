// @spec SPEC-016-1-architecture-4-couches @spec SPEC-016-2-design-system-rgaa
// @intent INTENT-016
...
export function sparkline(values, opts = {}) {
  ...
  const label = escape(opts.label || 'Sparkline');
  if (!values || values.length === 0) return `<svg class="spark" role="img" aria-label="Données non disponibles" width="${w}" height="${h}"></svg>`;
  if (values.length === 1) {
    return `<svg class="spark" role="img" aria-label="${label}" width="${w}" height="${h}"><circle cx="${w / 2}" cy="${h / 2}" r="3" /></svg>`;
  }
  ...
  return `<svg class="spark" role="img" aria-label="${label}" width="${w}" height="${h}" viewBox="0 0 ${w} ${h}"><polyline points="${points}" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></svg>`;
}
