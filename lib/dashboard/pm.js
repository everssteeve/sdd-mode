...
function sectionsDetails(sections) {
  ...
  const blocs = items.map((it) => {
    ...
    // Préserver les retours à la ligne sans interpréter le Markdown (sécurité).
    return `<div class="pm-section"><strong>${escape(it.label)}</strong><pre>${escape(v)}</pre></div>`;
  }).join('');
