...
export function blocCouverturePrd(donnees) {
  ...
  if (!c || (c.personas.length === 0 && c.userStories.length === 0)) return '';
  ...
  const t = c.totaux;
  return `<section>
    <h2>Couverture PRD <span class="count">${t.personasCouvertes}/${t.personas} personas · ${t.userStoriesCouvertes}/${t.userStories} US</span></h2>
    <p class="muted" style="font-size:.85rem">Pour chaque persona et user story déclarée dans ${c.fichier ? lienSource(c.fichier) : '<code>PRD.md</code>'}, nombre d'Intents qui le servent (matching frontmatter explicite + heuristique sur POURQUOI / POUR QUI / OBJECTIF).</p>
    ${pers ? `<h3>Personas</h3>
    <table>
