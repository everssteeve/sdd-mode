...
export const CATALOGUE = {
  ...
  // (SPEC-033-3) gate-precheck — pré-contrôle déterministe de l'Execution Gate.
  // Miroir de `.aiad/schema/verdicts/gate-precheck.schema.json`.
  'gate-precheck': {
    summary: 'Pré-contrôle déterministe de la Gate — périmètre d\'exécution de l\'agent, actions irréversibles, seuil d\'arrêt (exit 0/1/2)',
    schema: { type: 'object', required: ['verdict', 'exitCode', 'spec', 'section', 'manques', 'lignesAbsentes', 'avertissements'], properties: { verdict: { type: 'string', enum: ['PASS', 'FAIL', 'JNSP'] }, exitCode: { type: 'integer', enum: [0, 1, 2] }, spec: { type: 'string' }, section: { type: ['string', 'null'], enum: ['absente', 'non-applicable', 'declaree', null] }, manques: { type: 'array', items: { type: 'string' } }, lignesAbsentes: { type: 'array', items: { type: 'string' } }, avertissements: { type: 'array', items: { type: 'string' } } } },
  },
};
    ...
    type: 'object', properties: {
      ...
      gaps: {
        ...
        properties: {
          ...
          codeSansSpec: { type: 'object', properties: { bloquant: { type: 'number' }, non_bloquant: { type: 'number' }, total: { type: 'number' }, items: { type: 'array', items: { type: 'object' } } } },
          codeSansTests: { type: 'array', items: { type: 'object' } },
          earsSpecsSansTests: { type: 'array', items: { type: 'object' } },
        },
      },
