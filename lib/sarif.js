...
export function rendreSarif(modele) {
  ...
  for (const f of modele.gaps.codeSansSpec.items) {
    results.push(resultat('AIAD-TRACE-005', f.severity === 'bloquant' ? 'error' : 'note',
      `Aucune annotation @spec dans ${f.path}.${f.severity === 'bloquant' ? ' ⚠ nouveau fichier' : ''}`,
      f.path, 1));
  }
