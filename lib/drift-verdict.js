...
export function listerGaps(m) {
  ...
  // Code sans @spec : bloquant si nouveau fichier (SPEC-022-2), non-bloquant sinon.
  for (const f of m.gaps.codeSansSpec.items) {
    gaps.push({ kind: 'code_without_spec', ref: f.path, file: f.path, blocking: f.severity === 'bloquant' });
  }
  return gaps;
}
