 ...
 * @returns {object}
 */
export const ACTIONS_VALIDES = ['created', 'modified', 'deleted', 'imported', 'archived'];

export function construireEvenement(input) {
  if (!ACTIONS_VALIDES.includes(input.action)) {
    throw new Error(`Action inconnue : "${input.action}". Valides : ${ACTIONS_VALIDES.join(', ')}.`);
  }
