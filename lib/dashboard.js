...
import { construirePageAujourdhui } from './dashboard/views/today.js';
import { blocInboxTriage } from './dashboard/views/inbox.js';
import { pageGraph } from './dashboard/graph.js';
import { enrichir } from './dashboard/model/index.js';
// @spec SPEC-017-3-digest-delta
import { ecrireSnapshotDigest } from './dashboard/digest-delta.js';
import { pageQa } from './dashboard/qa.js';
import { pagePm } from './dashboard/pm.js';
import { genererPagesIntents } from './dashboard/intent-page.js';
// @spec SPEC-017-4-pages-detail-spec
import { genererPagesSpecs } from './dashboard/spec-page.js';
import { pageAdrs } from './dashboard/adrs.js';
import { pageLegal } from './dashboard/legal.js';
import { pageOnboarding } from './dashboard/onboarding.js';
import { pageKanban } from './dashboard/kanban.js';
import { pageSre } from './dashboard/sre.js';
import { pageDpo } from './dashboard/dpo.js';
...
const RENDERERS = {
  today: { titre: "Aujourd'hui", sous: null, render: construirePageAujourdhui },
  ...
  inbox: { titre: 'Inbox', sous: 'Triage facts & drifts', render: blocInboxTriage },
  eco: { titre: 'Impact écologique', sous: 'CO₂ estimé sur 30 sessions — EcoLogits (non certifié)', render: pageEco },
};
...
export async function dashboard(racineProjet, options = {}) {
  ...
  const donnees = await enrichir(racineProjet);
  ...
  // (#017-4) Pages individuelles par SPEC (spec-NNN-slug.html).
  try {
    const pagesSpec = genererPagesSpecs(donnees, { outDir, layout });
    for (const f of pagesSpec) ecrits.push(join(outDir, f));
  } catch (e) { console.warn('[spec-page] génération partielle ou échouée :', e?.message || e); }
  ...
  // (#017-3) Snapshot digest post-génération — persiste l'état courant pour
  // calcul du delta au prochain run.
  try {
    ecrireSnapshotDigest(racineProjet, donnees);
  } catch { /* snapshot digest non-fatal */ }
...
async function checkDashboard(racineProjet) {
  ...
  let donnees;
  try {
    donnees = await enrichir(racineProjet);
    donnees.publicUrl = '';
    donnees.sourceBase = '';
  } catch (err) {
