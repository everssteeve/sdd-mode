...
/**
 * Catalogue des verdicts disponibles avec seuils (pct ≥ seuil → verdict).
 * Liste triée décroissante : le premier match s'applique.
 */
export const VERDICTS = [
  { label: 'Excellent', seuil: 0.85, sym: '✓', couleur: 'vert' },
  { label: 'Bon', seuil: 0.7, sym: '✓', couleur: 'cyan' },
  { label: 'À retravailler', seuil: 0.5, sym: '⚠', couleur: 'jaune' },
  { label: 'Insuffisant', seuil: 0, sym: '✗', couleur: 'rouge' },
];
...
export function verdict(total, max) {
  ...
  for (const v of VERDICTS) {
    if (pct >= v.seuil) return { sym: v.sym, label: v.label, couleur: v.couleur };
  }
