...
/**
 * Exécute `afterCommand` si défini, puis le moteur de chaînage automatique.
...
export async function executerAfter(racine, ctx) {
  ...
  try {
    ...
    // Moteur de chaînage — appelé après le user hook uniquement si celui-ci n'a pas throwé.
    await evaluerChainage(racine, ctx);
    return { called };
  } catch (err) {
    return { called, error: err };
  }
}
