...
**La vérification se conçoit, elle ne s'achète pas (v1.9).** AIAD fait le pari que la qualité de la vérification dépend d'abord du design du processus — ce qu'on vérifie, quand et par qui — plutôt que de la puissance du modèle qui relit. C'est une position, pas un résultat démontré : les indices convergent (dossier `https://github.com/everssteeve/sdd-mode/blob/main/docs/legitimation/verification-evidence.md`), aucun ne suffit seul. Elle se traduit en quatre règles.
