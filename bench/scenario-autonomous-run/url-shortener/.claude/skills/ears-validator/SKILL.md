...
| ≥ 1 critère JNSP | Critère 2 = **JNSP** — Gate **INCONNUE** (fail-closed) |

En mode indicatif, le résultat est purement informatif et n'altère pas le score SQS.

## État JNSP par critère

Un critère est `JNSP` (pas PASS, pas FAIL) quand le linter ne peut pas
trancher sans information externe :

- Vocabulaire métier inconnu — un terme apparaît mais aucun glossaire
  n'est disponible pour juger « sujet explicite » (R4) ou « verbe
  observable » (R5).
- Quantification présente mais l'unité de mesure est ambiguë (« 200 » sans
  unité : ms, requêtes, octets ?).
- Critère renvoie à un fichier externe (« voir contrat /api/X.yaml ») et
  ce fichier n'a pas pu être lu dans cette session.

Forme dans la sortie :

```
CA-005 : "WHEN payment fails, the system SHALL retry up to 200."
  → ❓ Event-driven · JNSP
     R6 (quantification)      : "200" sans unité (ms ? tentatives ?)
  → Question : préciser l'unité de la limite 200
```

Le compteur JNSP est distinct de `violations` dans le résumé final :

```
Score EARS         : C / Y critères conformes  (X %)
Critères JNSP      : J / Y                     (Z %)   ← nouveau
Score testabilité  : T / Y critères testables  (X %)
Violations totales : N (R1=a, R2=b, R3=c, R4=d, R5=e, R6=f, R7=g)
```
