...
3. Trois verdicts possibles :
   - **PASS** — les 5 points sont validés.
   - **FAIL** — au moins un point identifié comme défaillant avec correction
     actionnable (ex. « auteur "Claude" → demander le nom humain »).
   - **JNSP** — un point ne peut pas être tranché par l'agent (cf. ci-dessous).
...
**Si JNSP :**
```
❓ HUMAN AUTHORSHIP — JNSP (intention non mûre)
Critère non scorable : [#X — intitulé]
Motif : [pourquoi tu ne peux pas trancher — ex. "l'humain bute sur
        POURQUOI MAINTENANT, signe que l'intention n'est pas prête"]
Question à l'humain : [reformulation actionnable]

Action : repousser la création d'INTENT, revenir quand la réponse est mûre.
```

## FAIL vs JNSP — distinguer les deux

| Cas | Verdict |
|-----|---------|
| Auteur = « Claude » | **FAIL** (correction connue : demander un nom humain) |
| Objectif sans chiffre alors que la métrique est évidente (« +20 % conversion ») | **FAIL** (proposer la reformulation) |
| L'humain ne sait pas répondre à « POURQUOI MAINTENANT » | **JNSP** (intention pas mûre, pas une erreur) |
| L'humain hésite entre 2 personas pour POUR QUI | **JNSP** (clarification métier requise) |
| CRITÈRE DE DRIFT formulé comme une intention (« le système doit être bon ») | **FAIL** (reformuler en signal observable) |

FAIL = erreur connue avec correction. JNSP = question ouverte qui doit
remonter à l'humain. Ne pas dégrader JNSP en FAIL pour faire avancer.

## Règles

- L'agent peut **aider à reformuler** un POURQUOI, mais jamais l'inventer.
- Si l'humain bute sur "POURQUOI MAINTENANT" ou "CRITÈRE DE DRIFT" → l'intention n'est pas mûre, repousser la rédaction (verdict JNSP, pas FAIL).
- Cette skill est non-négociable : elle s'applique avant TOUTE création d'artefact stratégique.
- Un JNSP est un **succès** de la skill — elle a détecté une zone qui doit rester humaine.
