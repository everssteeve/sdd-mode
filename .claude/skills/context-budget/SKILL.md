...
?/5 JNSP       → ≥ 1 métrique non mesurable (voir ci-dessous)
```

## Score JNSP — `?/5`

Quand au moins une métrique M1–M5 n'est **pas mesurable** dans le
contexte courant, le score global sort en `?/5` plutôt qu'en `0/5`.
C'est distinct du verdict CRITIQUE :

| Métrique | Déclencheur JNSP |
|----------|------------------|
| M1 Taux utile | Pas d'instrumentation du contexte effectivement lu par l'agent. |
| M2 Relances | Session précédente non logée (`.aiad/metrics/` absent). |
| M3 Ratio est/réel | SPEC §6 « Budget contexte » non rempli — pas d'estimation à comparer. |
| M4 Cohérence | AGENT-GUIDE absent ou n'expose pas de contraintes mesurables. |
| M5 Durée | Pas de timestamp de début / fin de session. |

Format de sortie :

```
M3 Ratio est/réel : JNSP   [❓]  motif=SPEC §6 non rempli
M5 Durée          : JNSP   [❓]  motif=session non timestampée

Score santé : ?/5 — JNSP sur 2 métrique(s)
Action : pour la prochaine session, remplir SPEC §6 et activer le log
        de session via .aiad/config.yml ; les autres métriques sont OK.
```

Ne pas remplacer JNSP par 0 — un 0 dit « la métrique est mauvaise », un
JNSP dit « la métrique n'a pas pu être calculée », et les actions
correctives sont différentes.
...
- Au-dessus de ≈ 50K tokens projetés → réduire AVANT de lancer l'agent. Le **50K est
  ...
  60–70 % de la fenêtre — un garde-fou opérationnel, pas une mesure.
