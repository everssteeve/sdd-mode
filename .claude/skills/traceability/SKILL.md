...
Gaps bloquants  : K
Gaps moyens     : L
Gaps UNKNOWN    : U      ← fichiers illisibles / annotations ambiguës

Verdict global : OK / GAP / UNKNOWN
```

## Verdict UNKNOWN (JNSP) — fail-closed

Quand la skill ne peut pas terminer le scan, le verdict est `UNKNOWN`
plutôt que `OK`. Déclencheurs :

- Fichier de code dont le contenu n'a pas pu être lu (permissions,
  binaire mal classifié, sous-module non-init).
- Annotation `@spec SPEC-NNN-N-slug` au format valide mais référençant
  une SPEC qui **n'existe pas** dans `.aiad/specs/` — la cohérence ne
  peut pas être vérifiée (orphan inversé, distinct d'un Intent orphelin).
- `.aiad/intents/_index.md` ou `.aiad/specs/_index.md` absent ou corrompu.
- Test annoté `@spec` mais le test n'a pas pu être exécuté pour confirmer
  qu'il couvre réellement la SPEC (CI sans pile de test).
  ...
  "unknownGaps": [
    {
      ...
      "motif": "fichier binaire (pas un fichier source attendu)",
      "question": "Whitelist ce fichier dans .aiad/hook-bypass.yml ou convertir en source texte ?"
    }
  ]
}
```

`UNKNOWN` est traité comme `GAP` par `--fail-on-gap` (exit ≠ 0) — la CI
bloque. C'est volontaire : on préfère un faux positif qui réclame une
clarification humaine plutôt qu'un faux négatif silencieux.
...
- Annotation pointant vers une SPEC absente → `UNKNOWN`, jamais `OK` silencieux.
